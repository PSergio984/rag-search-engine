...
MOVIE_EMBEDDINGS_PATH = Path(__file__).resolve().parent.parent.parent / "cache" / "movie_embeddings.npy"
CHUNK_EMBEDDINGS_PATH = Path(__file__).resolve().parent.parent.parent / "cache" / "chunk_embeddings.npy"
CHUNK_METADATA_PATH = Path(__file__).resolve().parent.parent.parent / "cache" / "chunk_metadata.json"
...
def embed_chunks_command() -> np.ndarray:
    movies = json.loads(Path(__file__).resolve().parent.parent.parent.joinpath("data", "movies.json").read_text())["movies"]
